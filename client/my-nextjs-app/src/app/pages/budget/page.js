...
const Budget = () => {
  ...
  const handleAddBudget = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/api/budgets/add', newBudget, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
  ...
  const handleUpdateBudget = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8080/api/budgets/update/${editingBudgetId}`,
        newBudget,
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
  ...
  const handleDeleteBudget = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/budgets/delete/${id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
